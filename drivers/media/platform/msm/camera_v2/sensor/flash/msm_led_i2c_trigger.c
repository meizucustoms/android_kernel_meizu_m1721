...
int32_t msm_flash_i2c_trigger_config(struct msm_led_flash_ctrl_t *fctrl,
                                     void *data) {
  int rc = 0;
  int i = 0;
...
static int msm_flash_pinctrl_init(struct msm_led_flash_ctrl_t *ctrl) {
  ...
  if (ctrl->pdev != NULL)
    ...
    flash_pctrl->pinctrl =
        devm_pinctrl_get(&ctrl->flash_i2c_client->client->dev);
  if (IS_ERR_OR_NULL(flash_pctrl->pinctrl)) {
    pr_err("%s:%d Getting pinctrl handle failed\n", __func__, __LINE__);
    return -EINVAL;
  }
  flash_pctrl->gpio_state_active = pinctrl_lookup_state(
      flash_pctrl->pinctrl, CAM_FLASH_PINCTRL_STATE_DEFAULT);
  ...
  flash_pctrl->gpio_state_suspend =
      pinctrl_lookup_state(flash_pctrl->pinctrl, CAM_FLASH_PINCTRL_STATE_SLEEP);
...
static int32_t msm_led_get_dt_data(struct device_node *of_node,
  ...
  fctrl->flashdata = kzalloc(sizeof(fctrl->flashdata), GFP_KERNEL);
  if (!fctrl->flashdata)
    return -ENOMEM;
  ...
  fctrl->pinctrl_info.use_pinctrl =
      of_property_read_bool(of_node, "qcom,enable_pinctrl");
  if (of_get_property(of_node, "qcom,flash-source", &count)) {
    ...
    for (i = 0; i < count; i++) {
      flash_src_node = of_parse_phandle(of_node, "qcom,flash-source", i);
      if (!flash_src_node) {
        pr_err("flash_src_node NULL\n");
        continue;
      }

      rc = of_property_read_string(flash_src_node, "linux,default-trigger",
      ...
      rc = of_property_read_u32(flash_src_node, "qcom,max-current",
                                &fctrl->flash_op_current[i]);
      if (rc < 0) {
        pr_err("failed rc %d\n", rc);
        of_node_put(flash_src_node);
    ...
    if (of_get_property(of_node, "qcom,max-current", &count)) {
      ...
      rc = of_property_read_u32_array(of_node, "qcom,max-current",
      ...
      for (count = 0; count < MAX_LED_TRIGGERS; count++)
        fctrl->torch_max_current[count] = fctrl->flash_max_current[count] >> 1;
    }
    ...
    if (of_get_property(of_node, "qcom,max-duration", &count)) {
      ...
      rc = of_property_read_u32_array(of_node, "qcom,max-duration",
    ...
    pr_err("%s: sensor_slave_addr is 0x%x, 0x%x, 0x%x\n", __func__, id_info[0],
           id_info[1], id_info[2]);
...
DEFINE_SIMPLE_ATTRIBUTE(ledflashdbg_fops, NULL, set_led_status, "%llu\n");
#endif
...
int32_t msm_i2c_torch_create_classdev(struct device *dev, void *data) {
  int rc;
...
int msm_flash_i2c_probe(struct i2c_client *client,
  ...
  snprintf(fctrl->msm_sd.sd.name, sizeof(fctrl->msm_sd.sd.name), "%s",
           id->name);
  ...
  if (!fctrl->flash_i2c_client->i2c_func_tbl)
    fctrl->flash_i2c_client->i2c_func_tbl = &msm_sensor_qup_func_tbl;
...
int msm_flash_probe(struct platform_device *pdev, const void *data) {
  ...
  fctrl->flash_i2c_client->cci_client =
  ...
  if (fctrl->flashdata->slave_info->sensor_slave_addr)
    cci_client->sid = fctrl->flashdata->slave_info->sensor_slave_addr >> 1;
  cci_client->retries = 3;
  cci_client->id_map = 0;

  if (!fctrl->flash_i2c_client->i2c_func_tbl)
    fctrl->flash_i2c_client->i2c_func_tbl = &msm_sensor_cci_func_tbl;
