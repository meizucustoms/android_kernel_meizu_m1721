			     ...
			     unsigned int flags, bool grab_handle)
{
	...
	pr_warn("%s: len %zu align %zu heap_id_mask %u flags %x\n", __func__,
	...
	if (IS_ERR(buffer)) {
		trace_ion_alloc_buffer_fail(client->name, dbg_str, len,
					    heap_id_mask, flags,
					    PTR_ERR(buffer));
		pr_warn("ION is unable to allocate 0x%zx bytes (alignment: 0x%zx) from heap(s) %sfor client %s\n",
					...
					size_t chunk_size, size_t total_size)
{
	...
	pr_warn("creating sg_table with %d chunks\n", n_chunks);
				       ...
				       enum dma_data_direction dir)
{
	...
	pr_warn("%s: syncing for device %s\n", __func__,
