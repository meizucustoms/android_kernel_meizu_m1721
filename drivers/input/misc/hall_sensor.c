...
static irqreturn_t hall_interrupt_handler(int irq, void *dev) {
  ...
  value = (gpio_get_value_cansleep(data->gpio) ? 1 : 0) ^ data->active_low;
  if (value) {
...
static int hall_input_init(struct platform_device *pdev,
  ...
  if (!data->hall_dev) {
    dev_err(&data->hall_dev->dev, "input device allocation failed\n");
    return -EINVAL;
  }
  ...
  if (err < 0) {
    dev_err(&data->hall_dev->dev, "unable to register input device %s\n",
            LID_DEV_NAME);
    return err;
  }
...
static int hall_config_regulator(struct platform_device *dev, bool on) {
  ...
  if (on) {
    ...
    if (regulator_count_voltages(data->vddio) > 0) {
      rc = regulator_set_voltage(data->vddio, data->min_uv, data->max_uv);
      if (rc) {
...
static int hall_parse_dt(struct device *dev, struct hall_data *data) {
  ...
  data->gpio = of_get_named_gpio_flags(dev->of_node, "linux,gpio-int", 0, &tmp);
...
static int hall_driver_probe(struct platform_device *dev) {
  ...
  err = devm_request_threaded_irq(&dev->dev, data->irq, NULL,
                                  hall_interrupt_handler, irq_flags,
                                  "hall_sensor", data);
  if (err < 0) {
...
MODULE_DEVICE_TABLE(platform, hall_id);
