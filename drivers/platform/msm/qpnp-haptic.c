...
int qpnp_hap_set(struct qpnp_hap *hap, int on);

static ssize_t qpnp_hap_vibrate_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(timed_dev, struct qpnp_hap,
	...
	qpnp_hap_set(hap, value);

	return count;
}

/* sysfs show vibrate */
static ssize_t qpnp_hap_vibrate_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "show for vibrate is not supported\n");
}
...
static struct device_attribute qpnp_hap_attrs[] = {
	...
	__ATTR(vibrate, (S_IRUGO | S_IWUSR | S_IWGRP),
			qpnp_hap_vibrate_show,
			qpnp_hap_vibrate_store),
};
