...
static void sm5424_stay_awake(struct sm5424_wakeup_source *source,
                              enum wakeup_src wk_src) {
  unsigned long flags;

  sm5424_dbg("wk_src: 0x%02x\n", wk_src);
...
static void sm5424_relax(struct sm5424_wakeup_source *source,
                         enum wakeup_src wk_src) {
  unsigned long flags;
...
static int sm5424_charger_get_property(struct power_supply *psy,
  ...
  struct sm5424_charger *sm5424 =
      container_of(psy, struct sm5424_charger, parallel_psy);
...
static int sm5424_charger_probe(struct i2c_client *client,
  ...
  sm5424->parallel_psy.property_is_writeable = sm5424_charger_is_writeable;
  sm5424->parallel_psy.num_properties = ARRAY_SIZE(sm5424_psy_properties);

  mutex_init(&sm5424->fcc_lock);
  mutex_init(&sm5424->irq_complete);
  spin_lock_init(&sm5424->ws.ws_lock);
  wakeup_source_init(&sm5424->ws.source, "sm5424");

  ret = power_supply_register(sm5424->dev, &sm5424->parallel_psy);
  if (ret) {
    pr_err("sm5424: Failed to register psy (%d).\n", ret);
    wakeup_source_remove(&sm5424->ws.source);
    wakeup_source_drop(&sm5424->ws.source);
    return ret;
  }
...
static int sm5424_charger_remove(struct i2c_client *client) {
  struct sm5424_charger *sm5424 = i2c_get_clientdata(client);

  wakeup_source_trash(&sm5424->ws.source);
